...
class FailureModeDetector:
    ...
    def detect_hallucination(self, content: str, delta_s: float) -> FailureDetection:
        return FailureDetection(
            self._modes["FM01"], delta_s > 0.7, min(delta_s / 1.0, 1.0), f"delta_s={delta_s:.3f}"
        )
    ...
    def detect_fact_conflict(self, content: str) -> FailureDetection:
        ...
        return FailureDetection(
            self._modes["FM02"], has_contradiction, 0.5 if has_contradiction else 0.0
        )
    ...
    def detect_circular_reasoning(self, content: str) -> FailureDetection:
        ...
        return FailureDetection(
            self._modes["FM10"], repeated and len(sentences) >= 4, 0.4 if repeated else 0.0
        )
    ...
    def detect_length_anomaly(self, content: str) -> FailureDetection:
        ...
        return FailureDetection(
            self._modes["FM11"], anomalous, 0.7 if anomalous else 0.0, f"length={length}"
        )
    ...
    def detect_repetition_excess(self, content: str) -> FailureDetection:
        ...
        if len(sentences) >= 4:
            ...
            if sentence_ratio < 0.5:  # 超过一半的句子重复
                return FailureDetection(
                    ...
                    True,
                    1.0 - sentence_ratio,
                    f"repeated_sentences={len(sentences) - len(unique_sentences)}",
                )
        ...
        return FailureDetection(
            self._modes["FM12"], ratio < 0.5, 1.0 - ratio, f"unique_ratio={ratio:.2f}"
        )
